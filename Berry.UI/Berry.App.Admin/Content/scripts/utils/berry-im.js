...
IMInit = function (option) {
    ...
    var defaults = {
        ...
        afterLinkSuccess: function () { //连接成功后
            ...
            imMethod.IMGetUnReadMsgNumList(function (num) {
                $messageWrap.find('.message-count').html(num);
                if (parseInt(num) > 0) {
                    IMIconflash(1);
                }
            });
        ...
        updateUserList: function (userAllList, onLineUserList) {
            ...
            for (var i in userAllList) {
                ...
                if (userJson.length > 0) {
                    ...
                    $.each(userJson, function (j, item) {
                        ...
                        html += '<div data-value="' + item.UserId + '" class="message-onename">' + item.RealName + ' <span>[' + userAllList[i].DepartmentName + ']</span></div>';
                        html += '</li>';
                    });
                    html += '</ul>';
                }
            ...
            $ul.html(html);
            //点击部门名称展开
            $messageWrap.find('.message-group a').click(function () {
        ...
        updateUserStatus: function (userId, isOnline) {
            var node = $messageWrap.find('.message-chatlist li').find('[data-value=' + userId + ']')[0];
            ...
            $messageWrap.find('.message-chatlist li').find('[data-value=' + userId + ']').prev('.message-oneface').find('img').attr('src', top.contentPath + '/Content/images/' + src + '');
    ...
    //关闭即时聊天
    $messageWrap.find('.message-close').click(function () {
        IMIconflash(0);
        $messageWrap.hide();
        $messageWindow.hide();
        $("#btn_message").show();
    ...
    //导航切换（联系人、讨论组、最近回话）
    $messageWrap.find('.message-nav li').click(function () {
        var tab_Id = $(this).attr('id');
        switch (tab_Id) {
            case "nav-contact-tab":
                $messageWrap.find("#message-contact-list").show();
                $messageWrap.find("#message-group-list").hide();
                $messageWrap.find("#message-last-list").hide();
                $(this).find('span').hide();
                break;
            case "nav-group-tab":
                $messageWrap.find("#message-group-list").show();
                $messageWrap.find("#message-contact-list").hide();
                $messageWrap.find("#message-last-list").hide();
                break;
            case "nav-last-tab":
                $messageWrap.find("#message-last-list").show();
                $messageWrap.find("#message-contact-list").hide();
                $messageWrap.find("#message-group-list").hide();
                $(this).find('span').show();
                break;
    ...
    //搜索（同事、讨论组、最近回话）
    $messageWrap.find('.message-body-search').find('.search-text').keyup(function () {
    ...
    function OpenChatWindow() {
        $messageWrap.find('.message-chatlist li').click(function () {
            $messageWindow.show();
            ...
            $messageWindow.find('.message-window-header .text').html('与 ' + name + ' 聊天中')
            $messageWindow.find('.message-window-chat').scrollTop($messageWindow.find('.message-window-chat')[0].scrollHeight);
            $messageWindow.find('.message-window-chat').find('.message-window-content').html('');
    ...
    function CloseChatWindow() {
        $messageWindow.find('.message-window-header .close').click(function () {
            $messageWindow.hide();
            defaults.windowId = "";
        });
    }
    ...
    function SendMessage() {
        var $textarea = $messageWindow.find('.message-window-send').find('textarea');
        $textarea.bind('keypress', function (event) {
            if (event.keyCode == "13") {
                var sendText = $(this).val();
                if (sendText) {
                    imMethod.IMSendToOne(options.toUserId, sendText);
                    $messageWindow.find('.message-window-send').html('<textarea autofocus placeholder="按回车发送消息,shift+回车换行"></textarea>');
                    SendMessage();
                }
            }
        });
    ...
    function GetMessage(fromUer, content, dateTime, flag) {
        if (fromUer != options.userId) {
            if (options.windowId == fromUer) {
                ...
                $messageWindow.find('.message-window-content').append(html)
                $messageWindow.find('.message-window-chat').scrollTop($messageWindow.find('.message-window-chat')[0].scrollHeight);
            }
            if (flag == undefined) {
                //统计未读消息总数量
                if (options.windowId != fromUer) {
                    var num = parseInt($messageWrap.find('.message-count').html());
                    num += 1;
                    if (num > 99) {
                        num = 99;
                    }
