$.fn.Tab = function (options) {
    ...
    function addTab(item) {
        ...
        if (tab.find("#" + item.id).length == 0) {
            ...
            if (item.url) {
                var $iframe = $("<iframe class=\"berry_iframe\" id=\"iframe" + item.id + "\" height=\"100%\" width=\"100%\" src=\"" + item.url + "\" frameBorder=\"0\"></iframe>")
...
loadnav = function () {
        ...
        $.each(authorizeMenuData, function (i) {
            ...
            if (row.ParentId == moduleId) {
                if (isbelowmenu(row.Id) == 0) {
                    _html += '<li><a id=' + row.Id + '><i class="fa ' + row.Icon + '"></i>' + row.FullName + '</a></li>';
                } else {
                    _html += '<li class="sub" title=' + (row.Description == null ? "" : row.Description) + '><a id=' + row.Id + '><i class="fa ' + row.Icon + '"></i>' + row.FullName + '</a>';
    ...
    function getchildnav(moduleId) {
        ...
        //console.log("       导航三菜单:" + moduleId)

        var _html = '<div class="sub-child"><ul>';
        $.each(authorizeMenuData, function (i) {
            var row = authorizeMenuData[i];
            if (row.ParentId == moduleId) {
                _html += '<li title=' + (row.Description == null ? "" : row.Description) + '><a id=' + row.Id + '><i class="fa ' + row.Icon + '"></i>' + row.FullName + '</a></li>';
                navJson[row.Id] = row;
            }
        });
        _html += '</ul></div>';
