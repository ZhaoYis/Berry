...
namespace Senparc.Weixin.MP.Containers
{
    ...
    public class AccessTokenContainer : BaseContainer<AccessTokenBag>
    {
        ...
        public static string TryGetAccessToken(string appId, string appSecret, bool getNewToken = false)
        {
            ...
            using (Cache.BeginCacheLock(LockResourceName, appId)) //同步锁--如果appSecret不同需要更新后重新取token
            {
                var bag = TryGetItem(appId); 
                if (bag.AppSecret != appSecret)
                {
                    bag.AppSecret = appSecret;
                    bag.AccessTokenExpireTime = DateTime.MinValue;
                    Update(bag, null);
                }
            } 
